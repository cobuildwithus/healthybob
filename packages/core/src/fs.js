...
export async function ensureDirectory(absolutePath) {
  await fs.mkdir(absolutePath, { recursive: true })
}

export async function ensureVaultDirectory(vaultRoot, relativePath) {
  const resolved = resolveVaultPath(vaultRoot, relativePath)
  await ensureDirectory(resolved.absolutePath)
  return resolved.relativePath
}

export async function readUtf8File(vaultRoot, relativePath) {
  const resolved = resolveVaultPath(vaultRoot, relativePath)
...
export async function readJsonFile(vaultRoot, relativePath) {
  const resolved = resolveVaultPath(vaultRoot, relativePath)
...
export async function writeVaultTextFile(vaultRoot, relativePath, content, options = {}) {
  const resolved = resolveVaultPath(vaultRoot, relativePath)
...
export async function writeVaultJsonFile(vaultRoot, relativePath, value, options = {}) {
  const content = `${JSON.stringify(value, null, 2)}\n`
  return writeVaultTextFile(vaultRoot, relativePath, content, options)
}

export async function appendVaultTextFile(vaultRoot, relativePath, content) {
  const resolved = resolveVaultPath(vaultRoot, relativePath)
...
export async function copyImmutableFileIntoVaultRaw(vaultRoot, sourcePath, relativePath) {
...
export async function walkVaultFiles(vaultRoot, relativeDirectory, options = {}) {
