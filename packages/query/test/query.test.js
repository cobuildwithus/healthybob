...
test("readVault assembles a stable read model from contract-shaped markdown and jsonl sources", async () => {
...
test("list helpers apply date, tag, text, and kind filters against contract data", async () => {
  ...
  try {
    ...
    assert.deepEqual(matchingJournal.map((record) => record.date), ["2026-03-10"]);
  } finally {
    await rm(vaultRoot, { recursive: true, force: true });
  }
});
