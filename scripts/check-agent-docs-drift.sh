...
release_artifacts_pattern="^(packages/cli/package.json|packages/cli/CHANGELOG.md|${release_notes_pattern#^})$"
...
package_json_version_only() {
  local path='packages/cli/package.json'
  local diff_lines=''
  local relevant=''
...
if [[ -n "$changed_files" ]] \
  && printf '%s\n' "$changed_files" | grep -Eq '^packages/cli/package.json$' \
...
then
  ...
  if [[ -z "$non_release_changes" ]] && package_json_version_only; then
    echo "Agent docs drift checks passed for packages/cli release artifacts only."
    exit 0
  fi
fi
