...
- `pnpm build`: executes the solution-style `tsc -b` build across the workspace package graph, with `packages/contracts` library emit ordered through `packages/contracts/tsconfig.build.json`.
- `pnpm typecheck`: validates shell syntax, typechecks repo-owned TS tools, runs each package’s local no-emit typecheck script, and verifies the shared `tsc -b` build, including `packages/inboxd`.
- `pnpm test`: runs docs drift enforcement, package verification for `contracts`/`cli`/`core`/`importers`/`inboxd`/`query`, and fixture/scenario integrity verification through `e2e/smoke/verify-fixtures.ts`.
...
- Repo-level checks execute canonical write/read paths in `core`, `importers`, `inboxd`, and `query`, and they build the CLI package through the same TypeScript workspace toolchain used for local development.
